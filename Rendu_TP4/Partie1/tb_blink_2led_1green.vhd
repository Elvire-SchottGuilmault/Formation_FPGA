...
entity tb_blink_2led_1green is
end tb_blink_2led_1green;

architecture Behavioral of tb_blink_2led_1green is
	...
	begin
	   ...
	   --À t=2 ms (+1 coup d'horloge), on doit avoir le premier clignotement, en rouge vu que le bouton n'est initialement pas appuyé
	    ...
	    --À t=10 ms, on devrait avoir un nouveau clignotement en vert, mais on en a déjà terminer un donc pas de nouveau clignotement
	    wait for 2 ms;
	    
	    assert (led_r = '0')
	        report "test failed - led_r != '0' at 10.000010 ms" severity failure;
	    assert (led_g = '0')
	        report "test failed - led_g != '0' at 10.000010 ms" severity failure;
	    assert false report "test OK, all led = '0' at 10.000010 ms" severity note;
	    
	    
	    --À t=11 ms, au milieu du troisième (non) clignotement, on relâche le bouton ce qui doit faire repasser la au rouge (qui donc s'allume)
	    wait for 999990 ns;
	    
	    assert (led_r = '0')
	        report "test failed - led_r != '0' at 11 ms" severity failure;
	    assert (led_g = '0')
	        report "test failed - led_g != '0' at 11 ms" severity failure;
	    assert false report "test OK, all led = '0' at 11 ms" severity note;
	    ...
	    assert (led_r = '1')
	        report "test failed - led_r != '1' at 11.000010 ms" severity failure;
	    assert (led_g = '0')
	        report "test failed - led_g != '0' at 11.000010 ms" severity failure;
	    ...
	    --À t=13 ms (milieu de période "éteinte"), on réappui sur le bouton
	    wait for 999990 ns;
	    
	    button_0 <= '1';
	    
	    --À t=14 ms, nouveau cycle de clignotement, en vert
	    wait for 1 ms;
	    wait for 10 ns;
	    
	    assert (led_r = '0')
	        report "test failed - led_r != '0' at 14.000010 ms" severity failure;
	    assert (led_g = '1')
	        report "test failed - led_g != '1' at 14.000010 ms" severity failure;
	    assert false report "test OK, only led_g = '1' at 14.000010 ms" severity note;
	    
	     
	     --À t=18 ms, on devrait avoir un nouveau cycle de clignotement, mais on en  a déjà eu un sur cette pression du bouton
	    wait for 4 ms;
	    
	    assert (led_r = '0')
	        report "test failed - led_r != '0' at 18.000010 ms" severity failure;
	    assert (led_g = '0')
	        report "test failed - led_g != '0' at 18.000010 ms" severity failure;
	    assert false report "test OK, all led = '0' at 18.000010 ms" severity note;
